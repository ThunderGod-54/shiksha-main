...
const [isSignUp, setIsSignUp] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isSignUp) {
      ...
      try {
        // This now calls Firebase createUserWithEmailAndPassword 
        // AND THEN registers the user in your Flask backend
        await apiService.register(email, password, userType);

        // After successful registration, most apps redirect to onboarding
        navigate('/onboarding');
      } catch (error) {
        // Firebase or Flask errors (e.g., "email-already-in-use")
        setError(error.message);
      }
    } else {
      try {
        // This calls Firebase signInWithEmailAndPassword 
        // AND THEN verifies the token with your Flask backend
        const data = await apiService.login(email, password);

        // Check if the backend user object says they are onboarded
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      // This triggers the Google Popup and syncs the ID Token with your Flask backend
      const data = await apiService.googleAuth(userType);

      // If new user, go to onboarding; if existing, go to dashboard
      ...
      <div className="auth-page">
        <div className="auth-card">
          ...
          <div className="social-icon-container">
            ...
            {/* You can keep or remove this button since we disabled the GitHub logic */}
            <button
